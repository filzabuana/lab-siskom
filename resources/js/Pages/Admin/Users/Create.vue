...
const props = defineProps({
    roles: Array // Menerima daftar nama role dari Controller
});
...
const form = useForm({
    ...
    password: '',
    roles: [],
    is_admin: false, // Akan terikat ke toggle amber
});
                ...
                <form @submit.prevent="submit" class="space-y-8 relative">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div class="space-y-2">
                            ...
                            <input v-model="form.email" type="email" required placeholder="user@student.untan.ac.id"
                                class="w-full bg-slate-50 dark:bg-white/[0.03] border-none rounded-2xl px-6 py-4 text-sm font-bold focus:ring-2 focus:ring-blue-500 dark:text-white transition-all outline-none">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <label v-for="role in roles" :key="role" class="group flex items-center gap-3 p-4 bg-slate-50 dark:bg-white/[0.02] border border-slate-100 dark:border-white/5 rounded-2xl cursor-pointer hover:bg-blue-50 dark:hover:bg-blue-500/5 transition-all">
                                <input type="checkbox" :value="role" v-model="form.roles"
                                       class="w-5 h-5 rounded-lg border-slate-300 text-blue-600 focus:ring-blue-500/20 transition-all cursor-pointer">
                    ...
                    <div class="p-6 bg-amber-500/5 border border-amber-500/10 rounded-[2rem] group transition-all hover:bg-amber-500/10">
                        <label for="is_admin" class="flex items-center gap-4 cursor-pointer">
                            ...
                            <div class="flex flex-col flex-1">
                                <span class="text-[10px] font-black text-amber-600 uppercase italic tracking-widest group-hover:tracking-[0.15em] transition-all leading-tight">Akses Superadmin</span>
                                <span class="text-[11px] font-bold text-slate-500 dark:text-zinc-400 mt-0.5">Berikan hak akses administrator utama (Root).</span>
                            </div>
                        </label>
                    </div>
                    ...
                    <div class="pt-4">
                        <button type="submit" 
                            ...
                            class="w-full py-5 bg-slate-900 dark:bg-blue-600 text-white text-xs font-black uppercase tracking-[0.3em] rounded-2xl shadow-xl hover:bg-blue-600 transition-all italic active:scale-95 shadow-blue-500/20 disabled:opacity-50">
                            {{ form.processing ? 'Menyimpan...' : 'Simpan Data Pengguna' }}
                        </button>
                    </div>
                </form>
            </div>
